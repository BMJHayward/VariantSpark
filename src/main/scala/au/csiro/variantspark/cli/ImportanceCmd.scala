...
class ImportanceCmd extends ArgsApp with SparkApp
  ...
  def run():Unit = {
    ...
    val rf:RandomForest = new RandomForest(RandomForestParams(oob=rfEstimateOob,
        ...
        mTry = if (rfMTry > 0) Math.sqrt(rfMTry) else 1,
        // TODO: add warning if outside of range
        nTryFraction = if (rfMTryFraction > 0 && rfMTryFraction < 1) totalVariables * rfMTryFraction else 0.1,
    ...
    implicit val rfCallback = new RandomForestCallback() {
      ...
      override   def onParamsResolved(actualParams:RandomForestParams) {
        ...
        echo(s"RF Params mTry: ${actualParams.mTry}")
        echo(s"RF Params mTryFraction: ${actualParams.nTryFraction.toLong}")
      }
